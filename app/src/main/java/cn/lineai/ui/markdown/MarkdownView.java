...
public final class MarkdownView extends LinearLayout {
    ...
    private TextView streamingTextView;
    ...
    public void setMarkdown(String markdown) {
        ...
        if (!plainMode && !streamingMode && value.equals(lastMarkdown)) {
            return;
        }
        plainMode = false;
        streamingMode = false;
    ...
    public void setStreamingText(String text) {
        ...
        if (streamingMode && value.equals(lastMarkdown)) {
            return;
        }
        plainMode = false;
        streamingMode = true;
        ...
        if (streamingTextView == null) {
            removeAllViews();
            streamingTextView = new TextView(getContext());
            streamingTextView.setTextColor(LineTheme.TEXT);
            streamingTextView.setTextSize(LineTheme.FONT_MD);
            streamingTextView.setLineSpacing(LineTheme.dp(getContext(), 2), 1.0f);
            streamingTextView.setTextIsSelectable(true);
            streamingTextView.setMovementMethod(LinkMovementMethod.getInstance());
            addView(streamingTextView, new LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT));
        }
        streamingTextView.setText(value);
    }
