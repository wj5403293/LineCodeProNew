...
public final class ToolCallBlockView extends LinearLayout {
    ...
    public void bind(ToolCall toolCall, ToolResult result) {
        ...
        if (ToolCallUtils.isHttpTool(name)) {
            String label = getContext().getString(R.string.tool_call_block_http);
            ToolCallGenericView view;
            if (getChildCount() > 0
                    && getChildAt(0) instanceof ToolCallGenericView
                    && ((ToolCallGenericView) getChildAt(0)).hasLabel(label)) {
                view = (ToolCallGenericView) getChildAt(0);
            } else {
                removeAllViews();
                view = new ToolCallGenericView(getContext(), label);
                addView(view, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
            }
            view.bind(toolCall, result);
            return;
        }
        String label = getContext().getString(R.string.tool_call_block_mcp);
        ToolCallGenericView view;
        if (getChildCount() > 0
                && getChildAt(0) instanceof ToolCallGenericView
                && ((ToolCallGenericView) getChildAt(0)).hasLabel(label)) {
    ...
    private String signature(ToolCall toolCall, ToolResult result) {
        ...
        if (result != null) {
            builder.append(result.getToolCallId()).append('|')
                    ...
                    .append(result.getReviewMessage()).append('|')
